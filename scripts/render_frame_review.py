...
def render(prod: Path) -> str:
    from director.shot_table import state_sentence

    table = read_artifact(prod, "shot_list.json")
    packages = {p.get("shot_id"): p for p in (read_artifact(prod, "gen_packages.json").get("packages") or [])}
    kf_data = read_artifact(prod, "keyframes.json")
    ...
    for shot in shots:
        ...
        is_fail = status == "fail"
        first_rel = _t(kf.get("first_frame_file")) or f"04-frames/{sid}.jpg"
        last_rel = _t(kf.get("last_frame_file")) or (f"04-frames/{sid}-last.jpg" if _t(pkg.get("keyframe_plan")) == "first_last" else "")
        first_miss = not (prod / first_rel).exists()
        last_miss = bool(last_rel) and not (prod / last_rel).exists()
        parent_rel = _parent_of(prod, first_rel)
        flags = []
        if shot.get("hardest"):
            flags.append("最难")
        ...
        chip_cls = " fail" if is_fail else (" nit" if note else "")
        nav.append(
            f"<a href='#{_esc(sid)}' class='chip{chip_cls}{' hard' if shot.get('hardest') else ''}' "
            f"data-shot='{_esc(sid)}' data-nit='{1 if note else 0}' data-fail='{1 if is_fail else 0}' data-hard='{1 if shot.get('hardest') else 0}'>{_esc(sid)}</a>"
        )
        if scene != last_scene:
            lock = _t(locks.get(scene))
            body.append(
                f"<section class='scene' id='{_esc(scene)}'><h2>{_esc(scene)} · {_esc(SCENE_LABELS.get(scene, shot.get('location_id')))}</h2>"
...
<div class="top"><div class="wrap">
  ...
  <h1>{_esc(title)}</h1>
  <p>父图 → 起幅 → 落幅，右侧是镜头表。「状态」一行是这一帧必须照做的连戏。最难：{_esc(hard_line)}。待重出：{_esc(fail_line)}。放过的 nits：{_esc(nit_line)}。</p>
  <div class="filters">
...
def main() -> int:
    ...
    parser.add_argument("--prod", default="productions/009-siem-reap")
    ...
    dest = prod / "REVIEW.html"
    dest.write_text(render(prod), encoding="utf-8")
    print(f"wrote {dest}")
    return 0
